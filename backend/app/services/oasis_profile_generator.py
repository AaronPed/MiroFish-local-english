...
class OasisProfileGenerator:
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """获取系统提示词"""
        base_prompt = "You are a social media user profile generation expert. Generate detailed, realistic personas for public opinion simulation, maximizing fidelity to real-world situations. You must return valid JSON format, and all string values must not contain unescaped newlines. Use Chinese."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity, maximizing fidelity to real-world situations.
...
2. persona: Detailed persona description (2000 words of plain text), must include:
   ...
   - Stance and viewpoints (attitude toward topics, what might anger/move them)
   - Unique characteristics (catchphrases, special experiences, personal hobbies)
   - Personal memory (an important part of the persona; describe this individual's connection to the event and their existing actions and reactions within it)
...
- Use Chinese (except the gender field must use English male/female)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for an institution/group entity, maximizing fidelity to real-world situations.
...
2. persona: Detailed account profile description (2000 words of plain text), must include:
   - Institution basic information (formal name, institution nature, founding background, main functions)
   ...
   - Institutional memory (an important part of the institution's persona; describe this institution's connection to the event and its existing actions and reactions within it)
...
- Institutional account posts must align with their identity positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            ...
            try:
                ...
                logger.error(f"Generate profile for entity {entity.name} failed: {str(e)}")
                # 创建一个基础profile
        ...
        logger.info(f"Starting parallel generation of {total} agent profiles (parallel: {parallel_count})...")
        print(f"\n{'='*60}")
        print(f"Starting agent profile generation - {total} entities, parallel: {parallel_count}")
        print(f"{'='*60}\n")
