...
class SimulationConfigGenerator:
    ...
    def generate_config(
        ...
        """
        logger.info(f"Start generating simulation config: simulation_id={simulation_id}, entities={len(entities)}")
...
- The user group is Chinese; the schedule should follow Beijing time habits
- 0-5 AM: almost no activity (activity coefficient 0.05)
- 6-8 AM: gradually becoming active (activity coefficient 0.4)
- 9-18 PM (work hours): moderately active (activity coefficient 0.7)
- 19-22 PM: peak period (activity coefficient 1.5)
- After 23 PM: activity decreases (activity coefficient 0.5)
- General pattern: low activity at midnight, increasing in the morning, moderate during work hours, peak in the evening
- **Important**: The example values below are for reference only. You need to adjust specific time slots based on the nature of the event and the characteristics of the participating groups
  - For example: student groups may peak at 21-23 PM; media is active all day; official institutions only during work hours
  - For example: sudden hot topics may lead to late-night discussions, so off_peak_hours can be shortened appropriately
...
- peak_hours (int array): Peak hours, adjusted based on the event's participating groups
...
- reasoning (string): Brief explanation of why this configuration was chosen"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format. The time configuration should align with Chinese daily routines."
...
- **Schedule aligns with Chinese daily routines**: almost inactive at 0-5 AM, most active at 19-22 PM
...
{{
    "agent_configs": [
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON. The configuration should align with Chinese daily routines."
